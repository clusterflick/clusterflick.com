...
/**
 * A short grid's footnote, saying what the date window is keeping out.
 *
 ...
 *
 * The lead-in is doing the job the empty state's heading does. Without it a
 * button appears under a grid that looks finished, with nothing saying why —
 * and it has to be a question rather than a heading, because the films above
 * it are a real answer and this only asks whether a longer one was expected.
 */
export default function HiddenResultsNotice({
  ...
  return (
    <div className={clsx(styles.notice, className)}>
      <p className={styles.lead}>Expecting more results?</p>
      <button type="button" className={styles.offer} onClick={onShowAll}>
        <span className={styles.text}>
          <span className={styles.headline}>Show all dates</span>
          <span className={styles.change}>
            {count.toLocaleString("en-GB")} more{" "}
            {count === 1 ? "film" : "films"}, next {days === null ? "on " : ""}
            {when}
          </span>
        </span>
