import Link from "next/link";

function VenueBlurb() {
  return (
    <section>
      <p>
        ODEON Greenwich is part of the ODEON cinema chain, one of the UK&apos;s
        oldest and largest cinema operators. Located on Bugsby Way in{" "}
        <Link href="/london-cinemas/greenwich/">Greenwich</Link>, southeast
        London, the venue features multiple screens with digital projection and
        modern sound systems, showing a wide range of mainstream blockbusters,
        new releases, and popular films. As a standard ODEON location, it offers
        online booking and concession stands serving the usual cinema snacks and
        drinks.
      </p>
    </section>
  );
}

export const seoDescription = "Southeast London multiplex in Greenwich";
export const seoHighlights = "blockbusters, new releases and popular films";

export default VenueBlurb;
